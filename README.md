...
- **长期记忆**：`双阶段处理` + `异步模块化` + `PGvector存储` + `Qwen3-1.7b/Qwen3-4b` + `Nomic-embed-text` 构成有状态记忆层，实现用户画像动态演进。
    ...
    - **更新阶段**：提取后的新记忆根据user_id进入pgvector分表中进行检索，每条记忆分别提取余弦相似度最高的三条记忆传递给本地模型 `Qwen3-4b` 生成新增/更新/删除或合并操作的建议，通过functional call生成入库命令，经过嵌入模型 `Nomic-embed-text` 向量化后入库，保证记忆不冗余不矛盾。
