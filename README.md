...
### 5、模型量化与微调
- **模型量化**：
    - `Qwen3-4b:Q4_K_M ()`: 查询改写模型经 `4bit量化`后，显存占用减少约4倍（约2.5GB），推理速度提升33%，精度几乎无损。
    - `Qwen3-1.7b:Q4_K_M ()`: 长期记忆提取模型经 `4bit量化`后，显存占用减少约3倍（约1.0GB），推理速度提升33%，精度几乎无损。
    - `BGE_reranker_v2`: 重排模型经 `ONNX导出 + 动态int8量化`，显存占用减少45%，推理速度提升接近一倍，精度损耗小于1%。
    - `nomic_text_embed_v1`: 嵌入模型经 `ONNX导出 + 动态int8量化`，显存占用减少45%，推理速度提升接近一倍，精度损耗小于1%。
