...
主要功能：
    1. 环境初始化：加载 .env 配置文件，初始化 LangFuse 监控。
    2. 服务连接：建立 Redis、Milvus、Neo4j、vLLM、ONNXRuntime 等外部服务的连接。
    3. 模块实例化：按顺序创建查询改写、高频缓存、短期记忆、长期记忆、检索工具、验证工具、翻译工具、在线搜索工具及 ReAct Agent 实例。
    4. 异步请求处理：定义 handle_request_async，实现完整的对话处理流程：
     - 查询改写 → 高频缓存检查 → 短期记忆加载 → 长期记忆预取 → 初始消息构造 → ReAct Agent 调用 → 异步长期记忆存储 → 缓存和短期记忆更新。
...
架构亮点：
    ...
    - 分层日志：集成 LangFuse 进行链路追踪，结合本地 logger 输出结构化日志到控制台，便于调试和监控。
