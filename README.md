...
- **长期记忆**：Mem0 + Milvus（长期记忆专用集合与知识库集合逻辑分离） 实现用户画像持久化，长期记忆数据包含**类型化记忆**（偏好/事实/事件）与**重要性排序**等元数据。
...
- **Agent 核心**：Qwen-plus 模型（API）驱动 ReAct 循环，自主决定是思考、观察、行动(调用工具或生成答案)，设置最大循环上限5轮避免无限循环。
    - 选型理由：当前用Qwen-plus API替代。后期通过云服务器升级计算资源，本地部署Qwen3-30b-a3b，3b激活参数MOE架构模型专用于ReAct Agent开发，具备自主规划循环调用各工具集能力。
- **工具集**：
...
| **Agent 模型** | Qwen-plus (API) | 大参数模型具备强大的指令跟随与多步规划能力，确保Agent精准解析输出、稳定执行工具调用。 |
...
| **嵌入模型** | nomic-embed-text (本地Ollama) | 开源轻量嵌入模型的性能标杆，统一用于知识库向量化、用户查询向量化及长期记忆向量化，确保语义检索一致性 |
| **长期记忆判断模型** | qwen3-1.7b (本地Ollama) | 轻量模型，快速决策记忆价值 |
| **API 服务** | FastAPI | 异步原生，高性能，支撑长期记忆异步写入不阻塞agent主程序 |
| **向量数据库** | Milvus 2.6 |IVF_RABITQ索引内存占用低且召回率高，sparse+BM25function实现全文检索，无需额外引入Elasticsearch降低数据库运维成本 |
| **图数据库** | Neo4j | 知识图谱实体关系存储，支持多跳验证 |
| **缓存+短期记忆数据库** | Redis-Stack | 内存数据库提供低延迟读写适合短期记忆存取，内置布隆过滤器支撑高频查询缓存过滤 |
| **消息队列** | Redis Stream | 支撑双数据库同步入库，适配当前数据量，无需额外引入Kafka |
| **模型量化** | ONNXRUNTIME | 相比Optimum方法更底层、可定制化量化范围更广 |
| **监控** | LangSmith + 本地logger | 自动提取所有由LangGraph开发的日志减少开发量，本地logger日志兜底 |
