...
### 5、本地模型量化与微调
- **模型量化**：
    - `Qwen3-4b:Q4_K_M (vLLM GPU部署)`: 查询改写模型经 `4bit量化`后，显存占用减少约4倍（约2.5GB），推理速度提升33%，精度几乎无损。
    ...
    - `Helsinki-NLP-opus-mt-en-zh`（ONNX CPU部署）: 英转中翻译模型经 `ONNX导出 + 动态int8量化（分别量化encoder层和decoder层）`，显存占用减少45%，推理速度提升接近一倍，精度损耗小于1%。 
