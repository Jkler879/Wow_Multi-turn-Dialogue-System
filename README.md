...
**ReAct Agent** + **Rag** 开发的**生产级多轮对话系统**，包含以下核心功能：
  ...
  - **ReAct自主规划思考**:  通过 `LanGraph状态图编排`规划多个工具集的自主调用，皆封装成MCP服务，包括`混合检索、实体关系验证、翻译等工具`。

  - **长短期记忆机制**:  通过`多级记忆（短期+长期）+ 查询改写(指代消解)`，在多轮对话中保持上下文连贯、指代准确、持久化存储用户画像。

  - **限流缓存熔断机制**:  通过三级令牌桶限流防止突发流量缓存穿透，高级查询归一化+布隆过滤器+计数器+互斥锁实现生产级缓存，防止缓存击穿及雪崩。

  - **GPU + CPU混合部署**:  通过vLLM推理优化部署两个本地模型 + ONNX推理优化部署3个本地模型，所有本地模型皆完成对应量化。
...
<p align="center">
  <a href="https://Jkler879.github.io/Wow_Multi-turn-Dialogue-System/architecture.html" target="_blank" rel="noopener noreferrer">
    <img src="./architecture_light.svg" alt="多轮对话 RAG + ReAct Agent 系统架构图" width="100%">
  </a>
</p>

<p align="center"><sub>点击图片打开可缩放版本（支持滚轮缩放、拖拽平移）</sub></p>
...
- **长期记忆**：`双阶段处理` + `异步模块化` + `PGvector存储` + `Qwen3-1.7b/Qwen3-4b` + `Nomic-embed-text` 构成有状态记忆层，实现用户画像动态演进。
    ...
    - **提取阶段**：每轮问答对生成后，本地模型 `Qwen3-1.7b` 判断当前问答对中是否有值得存储的记忆，通过 `Prompt` 约束返回最多3条最重要的新记忆并携带记忆类型(偏好/事实/事件)和重要性（0-1）标签。
    - **更新阶段**：提取后的新记忆根据user_id进入pgvector分表中进行检索，每条记忆最多提取余弦相似度最高的三条记忆传递给本地模型 `Qwen3-4b` 生成新增/更新/删除/合并的操作建议，通过functional call生成sql语句，经过嵌入模型 `Nomic-embed-text` 向量化后入库，保证记忆不冗余不矛盾。
...
- **模型量化**：
  ...
  **GPU 推理**（vLLM，OpenAI 兼容接口）
  ...
  |------|---------|---------|---------|------|
  | Qwen3-4B | vLLM 端口 8001 | AWQ int4官方量化版本 | ~8GB | 查询改写 / 长期记忆新旧记忆比对 / 反思生成 |
  | Qwen3-1.7B | vLLM 端口 8002 | 官方原版本 | 3.22GB | 长期记忆新记忆提取：提取每轮问答对有价值的记忆 |
